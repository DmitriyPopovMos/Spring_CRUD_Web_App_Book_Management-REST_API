...
3. Хранить поля: id, vendorCode, title, year, brand, stock, price. Все данные сохраняются в базу данных.
    В проекте использована БД PostgreSQL (установлена сортировка в порядке возрастания по полю "id")
...
5. Поддерживать пагинацию и фильтрацию по title, brand, year
    В проекте настроены фильтры для REST API:
    (moreThan, lessThan, equal, like, in, between)

    В проекте настроена пагинация для (Возврат постраничных данных, Сортировка, Обработка ошибок)
